...
local on_attach = function(client, bufnr)
  ...
  wk.add({
    { "<C-k>", function() vim.lsp.buf.signature_help() end, buffer = 4, desc = "Signature Help", remap = false },
    { "<leader>d", group = "Code Definitions" },
    { "<leader>D", function() vim.lsp.buf.type_definition() end, buffer = 4, desc = "Type Definition", remap = false },
    { "<leader>ca", function() vim.lsp.buf.code_action() end, buffer = 4, desc = "Code Action", remap = false },
    { "<leader>e", function() vim.diagnostic.open_float() end, buffer = 4, desc = "Line Diagnostics", remap = false },
    { "<leader>F", function() vim.lsp.buf.format() end, buffer = 4, desc = "Format all files", remap = false },
    { "<leader>q", function() vim.lsp.diagnostic.set_loclist() end, buffer = 4, desc = "Diagnostics in Location List", remap = false },
    { "<leader>r", group = "Code Action: Rename" },
    { "<leader>ro", function() vim.lsp.buf.rename() end, buffer = 4, desc = "Rename Object", remap = false },
    { "<leader>w", group = "Workspace" },
    { "<leader>wa", function() vim.lsp.buf.add_workspace_folder() end, buffer = 4, desc = "Add Folder to Workspace", remap = false },
    { "<leader>wl", function() print(vim.inspect(vim.lsp.buf.list_workspace_folders())) end, desc = "List workspace Folders", remap = false },
    { "<leader>wr", function() vim.lsp.buf.remove_workspace_folder() end, buffer = 4, desc = "Remove Workspace Folder", remap = false },
